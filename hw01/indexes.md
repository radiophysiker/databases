...
3. **История транзакций**: Последние 20 операций пользователя. Поиск истекающих покупок для уведомлений
...
- **Кардинальность**: Средняя (365-366 комбинаций месяц-день)
...
- **Особенность**: Частичный индекс для активных задач
- **Тип запроса**:
...
```sql
-- Индекс для истории покупок пользователя
...
- **Кардинальность**: Средняя
- **Зачем**: Операции «покажи задания на сегодня/на дату X»
- **Тип запроса**:
...
-- остальные ограничения на стороне приложения
```
...
### 3.3. Дополнительные бизнес-ограничения
